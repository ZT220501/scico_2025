...
class ParallelSemiProxJacobiADMML2PlusReg(Optimizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def _make_x_update_fn(A_i, g_i):
            @jax.jit
            def x_update_single(x_i, Ax_i, z_i, λ_i, τ):
                grad = self.ρ * A_i.T(Ax_i - z_i - λ_i / self.ρ)
                return g_i.prox(x_i - 1 / τ * grad, self.regularization / τ)
            return x_update_single
        self._x_update_fns = [_make_x_update_fn(Ai, gi) 
                               for Ai, gi in zip(self.A_list, self.g_list)]
    ...
    #     # x-update step.
    #     print("Updating the x variable on GPU ", worker_index, "...")
    ...
    #     print("Update on GPU ", worker_index, " completed.")
    ...
    def _itstat_extra_fields(self):
        ...
        itstat_fields = {"Prml Rsdl": "%9.3e", "Dual Rsdl": "%9.3e", "Regularization": "%9.3e", "Constraint": "%9.3e", "Proximal Parameter": "%9.3e"}
        itstat_attrib = ["norm_primal_residual()", "norm_dual_residual()", "regularization_value()", "constraint_value()", "proximal_parameter()"]
