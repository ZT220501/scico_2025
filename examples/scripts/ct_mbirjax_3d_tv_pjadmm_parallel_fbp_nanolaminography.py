...
def pjadmm_parallel_fbp_nanolaminography_test(
    ...
    full_img = preprocess_nanolaminography_image(img_path=img_path)
    ...
    sinogram = A_full @ full_img        # The sinogram is stored onto the first GPU.
    # Create the initial guess using the filtered back projection on the full sinogram.
    # Notice that the device of the initial guess needs to be on the CPU to avoid memory constraints.
    initial_guess = A_full.fbp_recon(sinogram)
    assert initial_guess.device == jax.devices('cpu')[0]

    # Save the initial guess if needed.
    if save_initial_guess:
        esults_dir = os.path.join(os.path.dirname(__file__), f'results')
        os.makedirs(results_dir, exist_ok=True)
        save_path = os.path.join(results_dir, f'ct_mbirjax_3d_tv_pjadmm_fbp_recon_real_data_initial_guess.png')
        save_recon_comparision(full_img, initial_guess, save_path)
    ...
    for i in range(row_division_num):
        for j in range(col_division_num):
            ...
            # Cut the initial guess into corresponding blocks.
            x0_block = full_img[:, roi_start_col:roi_end_col, roi_start_row:roi_end_row]
