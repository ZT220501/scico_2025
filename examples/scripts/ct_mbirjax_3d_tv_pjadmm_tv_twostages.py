...
def pjadmm_test(Nx=128, Ny=256, Nz=64, row_division_num=2, col_division_num=2, do_block_recon=True):
    ...
    for tv_weight_second_stage in [1e4, 1e5, 1e6, 1e7, 1e8]:

        print(f"tv_weight_second_stage: {tv_weight_second_stage} reconstruction starts\n")
        """
        ...
        # TODO: Maybe not changing the TV weight, but changing the ρ and τ to fit in the TV weight?
        # Take the ground truth, calculate its TV norm; find out which tv_weight will have the case
        # where constant gives better objective value than the ground truth image objective.
        # The answer might be projector dependent. (Maybe?)
        ρ = 1e-4
        τ = 0.03
        ...
        save_path = os.path.join(results_dir, f'ct_mbirjax_3d_tv_pjadmm_tv_2stages_recon_{n_projection}views_{Nx}x{Ny}x{Nz}_tv_weight_{tv_weight_second_stage}.png')
