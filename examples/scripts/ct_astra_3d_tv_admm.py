...
try:
    ...
    # Check if objects are JAX arrays
    if hasattr(y, 'device'):
        print(f"y device: {y.device()}")
    if hasattr(tangle, 'device'):
        print(f"tangle device: {tangle.device()}")
        
except ImportError:
    print("JAX not available for device checking")
