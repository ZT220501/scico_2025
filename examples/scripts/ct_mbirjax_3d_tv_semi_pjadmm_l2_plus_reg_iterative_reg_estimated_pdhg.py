...
def pjadmm_parallel_fbp_parallel_noisy_test(
    ...
    print(f"ρ: {ρ}, τ: {τ}, regularization: {regularization}, γ: {γ}, correction: {correction}, α: {α}, maxiter: {maxiter}, regularization_type: {regularization_type}, maxiter_pdhg: {maxiter_pdhg}")
    ...
    # Save the figure
    if tau_decrease:
        results_dir = os.path.join(os.path.dirname(__file__), f'results/semi_pjadmm_parallel_{regularization_type}_fbp_noisy_sinogram_snr{sinogram_snr}_{row_division_num}_{col_division_num}_N_sphere{N_sphere}_l2_plus_reg_iterative_reg_estimated_pdhg')
    else:
        results_dir = os.path.join(os.path.dirname(__file__), f'results/semi_pjadmm_parallel_{regularization_type}_fbp_noisy_sinogram_snr{sinogram_snr}_{row_division_num}_{col_division_num}_N_sphere{N_sphere}_l2_plus_reg_iterative_reg_estimated_pdhg_no_tau_decrease')
    os.makedirs(results_dir, exist_ok=True)
    save_path = os.path.join(results_dir, f'ct_mbirjax_3d_{regularization_type}_semi_pjadmm_l2_plus_reg_iterative_reg_estimated_pdhg_parallel_fbp_recon_noisy_{n_projection}views_{Nx}x{Ny}x{Nz}_foam_ρ{ρ}_τ{τ}_regularization{regularization}_gamma{γ}_maxiter{maxiter}_maxiter_pdhg{maxiter_pdhg}.png')
    fig.savefig(save_path)   # save the figure to file
...
if __name__ == "__main__":    
    ...
    print("="*100)
    print("3D TV-Regularized Sparse-View CT Reconstruction (Proximal Jacobi L2+TV ADMM Solver) using MBIRJAX, iterative regularization, estimated PDHG parameters")
    print("="*100)
    ...
    # Run the test
    print("\n" + "="*80)
    print("TEST: Block Proximal semi-Jacobi ADMM with iterative regularization, estimated PDHG parameters test")
    print("="*80)
